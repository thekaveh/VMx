...
- **`spec/` is the source of truth.** 23 numbered markdown chapters (`00-overview.md` … `22-discriminator-vm.md`), 84 ADRs, four JSON fixtures, current version in `spec/VERSION` (3.4.0). Behavior changes start here.
...
- **`spec/12-conformance.md` enumerates 301 normative test IDs** — 296 library IDs (`LIFE-001`, `HUB-013`, `CVM-009`, `BLD-006`, `GRP-011`, `HIER-018`, `NOTIF-017`, `COMP-025`, `COMP-026`, `DISC-006`, …) plus 5 `THEME-00x` scenario IDs that live in the flagship example apps. C#, Python, TypeScript, Swift, and Rust each implement all 296 library IDs under their conformance test trees. `tools/check-conformance-coverage.py` enforces 100% coverage for all five full-parity flavors in CI.
