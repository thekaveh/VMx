...
- **`spec/` is the source of truth.** 24 numbered markdown chapters (`00-overview.md` … `23-async-resource-vm.md`), 115 ADRs, four JSON fixtures, supporting schemas, current version in `spec/VERSION` (3.22.0). Behavior changes start here.
...
- **`spec/12-conformance.md` enumerates 400 normative test IDs** — 395 library IDs (`LIFE-001`, `HUB-013`, `CVM-010`, `SUBV-004`, `AGCH-010`, `SRCH-007`, `ARES-011`, `DISP-014`, `FORM-030`, `COL-064`, `BLD-006`, `GRP-011`, `HIER-030`, `NOTIF-017`, `COMP-041`, `DISC-006`, …) plus 5 `THEME-00x` scenario IDs that live in the flagship example apps. C#, Python, TypeScript, Swift, and Rust each implement all 395 library IDs under their conformance test trees. `tools/check-conformance-coverage.py` enforces 100% coverage for all five full-parity flavors in CI.
