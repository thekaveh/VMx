...
VMx is **one language-neutral specification with five idiomatic flavors**. All five are catalog-complete for the 403 library conformance IDs and aligned on the canonical public concepts and observable behavior. Rust's completed convergence evidence is recorded in `docs/maintenance/2026-07-16-rust-capability-parity.md`; ADR-0120 retains its ownership-safe sender-identity idiom. Naming remains idiomatic (PascalCase C#, snake_case Python and Rust methods, camelCase TypeScript and Swift — codified in `spec/ADRs/0006-idiomatic-api-per-language.md`).

- **`spec/` is the source of truth.** 24 numbered markdown chapters (`00-overview.md` … `23-async-resource-vm.md`), 129 ADRs, four JSON fixtures, supporting schemas, current version in `spec/VERSION` (3.23.0). Behavior changes start here.
...
- **`spec/12-conformance.md` enumerates 408 normative test IDs** — 403 library IDs (`LIFE-001`, `HUB-013`, `CVM-010`, `SUBV-004`, `AGCH-010`, `SRCH-007`, `ARES-011`, `DISP-014`, `FORM-030`, `COL-064`, `BLD-006`, `GRP-011`, `HIER-030`, `NOTIF-017`, `COMP-041`, `DISC-006`, …) plus 5 `THEME-00x` scenario IDs that live in the flagship example apps. C#, Python, TypeScript, Swift, and Rust each implement all 403 library IDs under their conformance test trees. `tools/check-conformance-coverage.py` enforces 100% catalog coverage for all five flavors in CI; coverage alone does not prove member-level parity.
...
4. Implement in every flavor that ships the area. Keep the public surface idiomatic per ADR-0006 and the normative concepts aligned; new flavor differences require explicit spec/ADR support. Hub `PropertyChangedMessage` names follow the flavor idiom (`"IsValid"` / `"is_valid"` / `"isValid"`); the collections `"Count"` channel is a spec-literal exception.
