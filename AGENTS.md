...
- **`spec/` is the source of truth.** 23 numbered markdown chapters (`00-overview.md` … `22-discriminator-vm.md`), 90 ADRs, four JSON fixtures, current version in `spec/VERSION` (3.10.0). Behavior changes start here.
...
- **`spec/12-conformance.md` enumerates 344 normative test IDs** — 339 library IDs (`LIFE-001`, `HUB-013`, `CVM-009`, `DISP-013`, `BLD-006`, `GRP-011`, `HIER-030`, `NOTIF-017`, `COMP-025`, `COMP-026`, `DISC-006`, …) plus 5 `THEME-00x` scenario IDs that live in the flagship example apps. C#, Python, TypeScript, Swift, and Rust each implement all 339 library IDs under their conformance test trees. `tools/check-conformance-coverage.py` enforces 100% coverage for all five full-parity flavors in CI.
