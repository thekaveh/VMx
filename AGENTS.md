...
- **`spec/` is the source of truth.** 23 numbered markdown chapters (`00-overview.md` … `22-discriminator-vm.md`), 96 ADRs, four JSON fixtures, current version in `spec/VERSION` (3.16.0). Behavior changes start here.
...
- **`spec/12-conformance.md` enumerates 359 normative test IDs** — 354 library IDs (`LIFE-001`, `HUB-013`, `CVM-010`, `SUBV-004`, `DISP-014`, `FORM-030`, `COL-055`, `BLD-006`, `GRP-011`, `HIER-030`, `NOTIF-017`, `COMP-025`, `COMP-026`, `DISC-006`, …) plus 5 `THEME-00x` scenario IDs that live in the flagship example apps. C#, Python, TypeScript, Swift, and Rust each implement all 354 library IDs under their conformance test trees. `tools/check-conformance-coverage.py` enforces 100% coverage for all five full-parity flavors in CI.
