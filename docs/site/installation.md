...
VMx has four stable source flavors plus a Rust preview flavor. The source tree
implements v3.1.0 for C#, Python, TypeScript, and Swift; Rust declares
`MIN_SPEC_VERSION = "3.1.0"` while its behavioral conformance assertions are
expanded. Public package availability can lag the source tree; check the flavor
README and registry before pinning a release.
...
| Rust       | 0.1.0 preview | crates.io package not published yet |
