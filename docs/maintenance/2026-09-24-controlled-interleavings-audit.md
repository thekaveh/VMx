...
| TypeScript | `langs/typescript/tests/globalSetup.ts`; `tests/dist-build/instanceof.test.ts`; lifecycle, notification, token-paging, and command suites                                                                 | Build ordering fixed; runtime schedules retain follow-up weak oracle | Global setup completes build and fixture/schema synchronization before workers; reruns await the previous run and invalidate generated modules without watcher feedback. Deferred lifecycle scheduling, synchronous delivery and explicit promises control runtime boundaries. Command event-loop drains still have the weak-oracle limitations below.                                                                                                                                                                          |
...
1. **Fixed build/test ordering:**
   [`langs/typescript/tests/globalSetup.ts`](../../langs/typescript/tests/globalSetup.ts)
   builds before test workers, replacing the build inside
   [`tests/dist-build/instanceof.test.ts`](../../langs/typescript/tests/dist-build/instanceof.test.ts).
   The old `beforeAll` ran npm's `prebuild` fixture/schema copies while other
   suites imported the same source JSON. During promotion, Windows reported
   `EBUSY` opening the consumer schema; that log does not prove which process
   held the OS lock. A separate boundary guard positively identified the
   worker-originated schema write. The setup orders this writer before readers.
   Watch reruns await the preceding run and rebuild current inputs. The watcher
   ignores generated outputs, and setup explicitly invalidates old and new
   output paths so cached ESM transforms cannot hide the rebuilt content.
   Automatic test selection is unchanged; manual reruns cover canonical spec
   inputs outside the import graph. Independent external builds are not
   serialized by this setup.
