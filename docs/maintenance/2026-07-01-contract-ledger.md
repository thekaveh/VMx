...
Last revalidated: **2026-07-25**. This branch-independent ledger records the
...
| ------------------ | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| C# runtime         | `langs/csharp/Directory.Packages.props`; project lockfiles      | `System.Reactive` and `Microsoft.Reactive.Testing` are `7.0.0`, with `System.Collections.Immutable` `10.0.10`; the library targets `netstandard2.0;net8.0`; both test suites execute on `net8.0;net9.0;net10.0`. Union-merged production-package Cobertura coverage cannot fall below 90% lines or 82% branches; test assemblies are excluded.      | `dotnet restore VMx.sln --locked-mode`; Release build/test with coverage; `tools/check-cobertura-threshold.py`; `dotnet format --verify-no-changes --no-restore`   |
| Python runtime     | `langs/python/pyproject.toml`; `langs/python/uv.lock`           | Python is `>=3.10`; `reactivex>=4.0.4` is the sole reactive runtime. The committed lock resolves stable `5.0.0`, validated by the full library and example suites, while wheel metadata retains the compatible range.                                                                                                                               | `uv sync --locked --all-extras`; pytest; Ruff check/format; `mypy --strict src/vmx`                                                                                |
...
| Rust runtime       | `langs/rust/Cargo.toml`; `Cargo.lock`                           | `vmx-rs` is `0.26.0`, implements spec `3.22.1`, and has MSRV Rust `1.88`; runtime dependencies are the current stable `serde 1.0.229` and `thiserror 2.0.19`; the VMx-owned hot-stream facade has no `rxrust` dependency. Test-only floors are `serde_json 1.0.145` (locked to `1.0.150`) and `pretty_assertions 1.4.1`.                            | `cargo search <crate> --limit 1` for both runtime crates; format; locked clippy/tests/docs/package; disposable-consumer build; empty `cargo tree -i rxrust`        |
