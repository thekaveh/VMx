...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nuget-csharp`                   | The environment exists with a required reviewer and tag policy, but permits self-review and has no `NUGET_USER` secret.                                                             | Disable self-review; add the nuget.org profile name only after its trusted-publishing policy is ready. C# publication remains blocked.                         |
...
| `npm-typescript` / `crates-rust` | Both environments have required reviewers and tag policies, permit self-review, and have no bootstrap token.                                                                        | Disable self-review. The missing token is correct only after first publication and trusted-publisher setup; otherwise first publication remains owner-blocked. |
