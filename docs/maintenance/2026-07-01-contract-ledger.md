...
Last revalidated: **2026-07-28**. This branch-independent ledger records the
...
| Rust runtime       | `langs/rust/Cargo.toml`; `Cargo.lock`                           | `vmx-rs` is `0.27.0`, implements spec `3.23.0`, and has MSRV Rust `1.88`; runtime dependencies are the current stable `serde 1.0.229` and `thiserror 2.0.19`; the VMx-owned hot-stream facade has no `rxrust` dependency. Test-only floors are `serde_json 1.0.145` (locked to `1.0.151`) and `pretty_assertions 1.4.1`.                            | `cargo search <crate> --limit 1` for both runtime crates; format; locked clippy/tests/docs/package; disposable-consumer build; empty `cargo tree -i rxrust`        |
