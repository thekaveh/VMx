...
Last revalidated: **2026-07-23**. This branch-independent ledger records the
...
| Release metadata parser      | `langs/python/pyproject.toml`; `langs/python/uv.lock`                         | `markdown-it-py 4.2.0` supplies current CommonMark 0.31.2 block semantics to the changelog release gate; release CI installs the locked `tools` extra before validation.    | `tools/tests/test_check_version_consistency.py`; clean `uv sync --locked --extra tools`; every supported `--release-tag` mode     |
