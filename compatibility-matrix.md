...
| ----- | --------------- | --------------- | --------------- | --------------- | ------------- |
| 3.5.x | 3.5.0[^current] | 3.5.0[^current] | 3.5.0[^current] | 3.5.0[^current] | 0.5.0[^rust] |
| 3.3.x | —               | —               | —               | —               | 0.3.0        |
| 3.2.x | —               | —               | —               | —               | 0.2.0        |
| 3.1.x | —               | 3.1.0           | —               | —               | 0.1.0        |
...
`makeConfirm` bridge) —
**304 of 304 library conformance IDs + 5 `THEME-00x` scenario IDs = 309 total
...
ADR-0084; +8 VM-collection move IDs per ADR-0085; THEME-001..005 covered by the
...
[^current]: 3.5.0 — current in-development source line and not yet tagged or
...
declares `MIN_SPEC_VERSION = "3.5.0"` and carries behavioral tests for all 304
...
> dependency floor. As built at HEAD both companions reference the `VMx` 3.5.0 core project, so a
> published companion would carry a `VMx >= 3.5.0` NuGet dependency — the raised floor to record in
