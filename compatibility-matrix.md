...
| ----- | --------------- | --------------- | --------------- | --------------- | ------------- |
| 3.7.x | 3.7.0[^current] | 3.7.0[^current] | 3.7.0[^current] | 3.7.0[^current] | 0.7.0[^rust] |
| 3.3.x | —               | —               | —               | —               | 0.3.0        |
| 3.2.x | —               | —               | —               | —               | 0.2.0        |
| 3.1.x | —               | 3.1.0           | —               | —               | 0.1.0        |
...
`makeConfirm` bridge) —
**316 of 316 library conformance IDs + 5 `THEME-00x` scenario IDs = 321 total
...
IDs per ADR-0086; +6 FormVM reset IDs per ADR-0087; THEME-001..005 covered by the
...
[^current]: 3.7.0 — current in-development source line and not yet tagged or
...
declares `MIN_SPEC_VERSION = "3.7.0"` and carries behavioral tests for all 316
...
> dependency floor. As built at HEAD both companions reference the `VMx` 3.7.0 core project, so a
> published companion would carry a `VMx >= 3.7.0` NuGet dependency — the raised floor to record in
