...
| ------ | ---------------- | ---------------- | ---------------- | ---------------- | ------------- |
| 3.11.x | 3.11.0[^current] | 3.11.0[^current] | 3.11.0[^current] | 3.11.0[^current] | 0.11.0[^rust] |
| 3.3.x  | —                | —                | —                | —                | 0.3.0         |
| 3.2.x  | —                | —                | —                | —                | 0.2.0         |
| 3.1.x  | —                | 3.1.0            | —                | —                | 0.1.0         |
...
`makeConfirm` bridge) —
**340 of 340 library conformance IDs + 5 `THEME-00x` scenario IDs = 345 total
...
\[^current\]: 3.11.0 — current in-development source line and not yet tagged or
...
declares `MIN_SPEC_VERSION = "3.11.0"` and carries behavioral tests for all 340
...
> dependency floor. As built at HEAD both companions reference the `VMx` 3.11.0 core project, so a
> published companion would carry a `VMx >= 3.11.0` NuGet dependency — the raised floor to record in
