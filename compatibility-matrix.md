...
| ----- | --------------- | --------------- | --------------- | --------------- | ------------- |
| 3.2.x | 3.2.0[^current] | 3.2.0[^current] | 3.2.0[^current] | 3.2.0[^current] | 0.2.0[^rust] |
| 3.1.x | —               | 3.1.0           | —               | —               | 0.1.0        |
...
`makeConfirm` bridge) —
**287 of 287 library conformance IDs + 5 `THEME-00x` scenario IDs = 292 total
...
ADR-0068..ADR-0079; +6 hub-transaction IDs per ADR-0082; THEME-001..005 covered by the
...
[^current]: 3.2.0 — current in-development source line and not yet tagged or
...
declares `MIN_SPEC_VERSION = "3.2.0"` and carries behavioral tests for all 287
...
> dependency floor. As built at HEAD both companions reference the `VMx` 3.2.0 core project, so a
> published companion would carry a `VMx >= 3.2.0` NuGet dependency — the raised floor to record in
