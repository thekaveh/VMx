...
- Background lifecycle scheduling failures now restore the prior status and
  clear transition admission, including foreground-completion rejection.
...
Implements `spec-v3.19.0` and keeps C# at full library parity: 380/380
conformance IDs covered.

### Added

- `SearchableState<T>` accepts an optional `IObservable<Unit> sourceChanged`
  signal that refreshes the current term after source mutations
...
- The declared current/minimum spec line advances to 3.14.0. The C# runtime
...
- `FormVM.SetModel` now uses model value equality to suppress a no-op candidate,
  then publishes exactly one `PropertyChangedMessage("Model")` after model,
  validation, and approve-command state settle (`FORM-030`, ADR-0092).
...
- `ComponentVMBase.Own(...)` registers `IDisposable` or `Action` resources for
...
Implements `spec-v3.7.0` and keeps C# at full library parity: 316/316
conformance IDs covered.
...
Implements `spec-v3.6.0` and keeps C# at full library parity: 310/310
conformance IDs covered.

### Added

- `RaiseCanExecuteChanged()` on `RelayCommand`, `RelayCommand<T>`, and
  `AsyncRelayCommand`, with exact additive and post-dispose semantics
  (`CMD-014..019`).

### Changed

- Built-in relay builders now return concrete relay commands while preserving
  the existing `ICommandBuilder` interfaces, so relay-specific capabilities do
  not require a cast.

## [3.5.0] — 2026-07-10

Implements `spec-v3.5.0` and keeps C# at full library parity: 304/304
conformance IDs covered.

### Added

- `IVmCollection<VM>` and `ISelectableVmCollection<VM>` capabilities shared by
  groups and composites.
- Atomic identity-preserving `Move(fromIndex, toIndex)` with one standard
  `NotifyCollectionChangedAction.Move` event (`COL-032..039`).

## [3.4.0] — 2026-07-10

Implements `spec-v3.4.0` and keeps C# at full library parity: 296/296
conformance IDs covered.
...
Implements `spec-v3.3.0` and keeps C# at full library parity: 290/290
conformance IDs covered.

### Added

- Protected `NotifyPropertyChanged` for emitting one hub
...
Implements `spec-v3.2.0` and keeps C# at full library parity: 287/287
conformance IDs covered.

### Added

- `ITransactionalMessageHub` and `MessageHub.Batch(Action)` for nested,
...
- `NullMessageHub` implements the additive transaction capability, executing
  the body while continuing to publish nothing.
