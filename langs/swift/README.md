...
**v3.22.1 source — total parity.** Covers **all 399 of 399** library conformance IDs
from `spec-v3.22.1` plus the 5 `THEME-00x` scenario IDs exercised by the
`examples/swift/notes-showcase/` flagship app (ADR-0067) = **404 total**, at
full parity with the C#, Python, and TypeScript UI-backed flavors. Rust has full
399-ID library parity and no UI scenario suite. Library IDs accumulated
...
ADR-0107; +1 canonical forwarding-ownership ID via ADR-0124; +1 aggregate
reconstruction transaction ID via ADR-0125):
...
## 5. Conformance — total parity (404)

This flavor implements **all 399 library conformance IDs** from the
...
**All 399 library conformance IDs are covered, and the 5 `THEME-00x` scenario IDs are covered by the `examples/swift/notes-showcase/` flagship. Swift is at total parity (404) with the C#, Python, and TypeScript UI-backed flavors. Rust has full library parity but no UI scenario suite.**
