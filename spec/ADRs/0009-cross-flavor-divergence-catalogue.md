...
| Collection `insert` out-of-range handling                      | `ArgumentOutOfRangeException` (BCL `List<T>.Insert`)                               | Clamps + normalizes negatives per stdlib `list.insert`, payload carries the effective index | `RangeError` (matching `removeAt`/`replace`)                             | Each flavor mirrors its native list's insert contract; spec/21 §3.2 only mandates the emitted payload carry the actual insertion index, which all three honor.             |
