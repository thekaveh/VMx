...
| Container batch API (`CompositeVM` / `GroupVM`)                | `BatchUpdate(): IDisposable`                                                       | `batch_update()` context-manager (`BatchUpdateHandle`)                                      | `batchUpdate(): BatchUpdateHandle`                                       | Each flavor returns an idempotent handle. C# uses `using`, Python uses `with`, and TypeScript uses `using` through `[Symbol.dispose]`; Swift's explicit-dispose divergence is recorded by ADR-0060.                                                                                                                     |
| `ObservableList` batch API                                     | `BatchUpdate(): IDisposable`                                                       | `batch_update()` context-manager                                                            | `withBatch(callback: () => void): void`                                  | TypeScript's standalone observable list retains the callback form; this is separate from the container handle API above.                                                                                                                                                                                                |
