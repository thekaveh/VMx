...
| Container batch API (`CompositeVM` / `GroupVM`)                | `BatchUpdate(): IDisposable`                                                       | `batch_update()` context-manager (`BatchUpdateHandle`)                                      | `batchUpdate(): BatchUpdateHandle`                                       | C#, Python, and TypeScript return idempotent handles; Swift returns an explicit-dispose handle per ADR-0060; Rust uses `batch_update(callback)` so unwind-safe scope ownership remains expressible without a borrowed handle. All forms preserve the same nesting, suppression, and outermost-reset behavior.           |
| `ObservableList` batch API                                     | `BatchUpdate(): IDisposable`                                                       | `batch_update()` context-manager                                                            | `withBatch(callback: () => void): void`                                  | TypeScript and Rust use callback scopes; Swift uses its container handle. These are idiomatic forms of the same nested suppression and outermost-reset contract.                                                                                                                                                        |
