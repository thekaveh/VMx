...
Capability concerns (per ADR-0010) layer in additively via opt-in interfaces
...
| `CompositionBase<M, VM, C, P>`           | Replaced       | Modern `CompositeVM<M, VM>` has two type params (model, VM) and no chain. Selection/CRUD/search now compose via the ADR-0014 / ADR-0016 helpers rather than inheriting from a base. |

### 2.2 What we DID bring forward, in adapted form

| Predecessor concept                                                  | Modern equivalent                                                                                                                             |
| -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `Construct/Destruct/Reconstruct/Dispose` lifecycle                   | `IComponentVM.Construct/Destruct/Reconstruct/Dispose` (ADR-0010 capability interfaces); enforced by the lifecycle state machine in chapter 02 |
| `TransformationProperty` derived values                              | `DerivedProperty<TValue>` (ADR-0011) with N source observables                                                                                |
| Capability micro-interfaces (`ISelectable` etc.)                     | Same names, now opt-in additively (ADR-0010)                                                                                                  |
| `CompositeCommand`/`DecoratorCommand`/`ConfirmationDecoratorCommand` | Same names, modern semantics (ADR-0012)                                                                                                       |
| `NotificationService`                                                | `INotificationHub` in opt-in sub-package (ADR-0013)                                                                                           |
| `SearchTerm` / `SearchPredicate` on composite                        | `SearchableState<TItem>` helper, opt-in (ADR-0014)                                                                                            |
| `IsExpanded` / `Expand` / `Collapse` on every VM                     | `ExpandableState` helper, opt-in (ADR-0015)                                                                                                   |
| `CreateNewCommand` / `DeleteCurrentCommand` etc.                     | `ModeledCrudCommands<M, VM>` helper, opt-in (ADR-0016)                                                                                        |
| `NullMessagingService`                                               | `NullMessageHub` + the broader null-object convention (ADR-0017)                                                                              |
