...
Widget-class discipline: ``compose()`` + ``on_mount()`` only. Selection
two-way and the list factory live in helper module-level functions.
"""
...
class NotesListView(Vertical):
    ...
    def on_mount(self) -> None:
        bind_property_two_way(
            self.query_one("#search_input", Input), "value", self._vm, "search_term"
        )
        bind_property_two_way(
            self.query_one("#starred_filter", Checkbox),
            "value",
            self._vm,
            "show_starred_only",
        )
        bind_observable_list(
            self.query_one("#notes_list", ListView), self._vm.inner, _note_list_item
        )
        bind_command(
            self.query_one("#page_first", Button), self._vm.move_to_first_page_command
        )
        bind_command(
            self.query_one("#page_prev", Button),
            self._vm.move_to_previous_page_command,
        )
        bind_command(
            self.query_one("#page_next", Button), self._vm.move_to_next_page_command
        )
        bind_command(
            self.query_one("#page_last", Button), self._vm.move_to_last_page_command
        )
        # ``page_label`` is a DerivedProperty[str] → bind via the derived bridge.
        bind_derived_property(
            self.query_one("#page_label", Static), "renderable", self._vm.page_label
        )
