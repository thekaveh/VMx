...
| 6   | `RelayCommand` + `AsyncRelayCommand` reactive            | `noteFormVM.approveCommand` / `denyCommand`; `noteVM.saveCommand` and the inner delete command await repository callbacks before success notification                                                                                                           |
