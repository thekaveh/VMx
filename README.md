...
Swift / Rust). All five source flavors cover the 400 library conformance IDs;
the flagship example apps cover 5 additional THEME scenario IDs for **405
...
- **Spec** — `spec/` is the source of truth: 24 markdown chapters, 127 ADRs,
  4 JSON fixtures, 405 conformance IDs, version pinned in `spec/VERSION`.
...
All five source flavors implement the 400 library conformance IDs. The flagship
example apps cover 5 additional THEME scenario IDs, for **405 total** tracked
...
- [`docs/content/getting-started/swift.md`](docs/content/getting-started/swift.md) —
  ...
  at total parity on the v3.23.0 source line; see `langs/swift/README.md` §5).
...
│   ├── ADRs/              architecture decision records (0001..0127)
...
│   ├── swift/             VMx Swift Package (v3.23.0 source, total parity — 400 library + 5 THEME)
...
- Per-flavor READMEs (status, install, API surface, dev commands):
  ...
  [`langs/swift/README.md`](langs/swift/README.md) (v3.23.0 source, total parity — 400 library + 5 THEME),
...
`spec/12-conformance.md` enumerates 405 normative test scenarios keyed by ID
(`LIFE-001`, `HUB-007`, `COMP-013`, `UTIL-002`, `CAP-020`, `DPROP-012`,
`NOTIF-010`, `DIA-001`, `FORM-001`, `COL-001`, `HIER-001`, `AGG-006`,
`AGCH-001`, `ARES-001`, `THEME-001`, …) — 400 library IDs plus 5 `THEME` scenario IDs. All five
flavors (C# / Python / TypeScript / Swift / Rust) implement the 400 library IDs under
...
four flavors are at **total parity: 400 library + 5 THEME = 405**; Rust has
full 400-ID library parity and intentionally has no UI scenario suite yet.
